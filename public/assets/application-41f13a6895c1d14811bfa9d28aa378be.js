...
(function() {
  ...
  Home.notify = function() {
    if (_.include(['/', '/home'], window.location.pathname)) {
      return Home.notifyLandingPage();
    }
  };

  Home.notifyLandingPage = function() {
    return noty({
      id: 'home-notify',
  ...
  Home.toggleDropdown = function(event) {
    ...
    if (cookie_data['mobile_nav_opened'] === void 0) {
      ...
      if (cookie_data['mobile_nav_opened'] === 'false') {
        return $('#mobile-nav-links').removeClass('hidden');
      } else {
        return $('#mobile-nav-links').addClass('hidden');
      }
    }
  };

  Home.slideToSection = function(event) {
    var id;
  ...
  Home.openRegisterModal = function() {
    ...
    return $('.modal.register-modal').modal();
  };

  Home.closeLoginModal = function() {
    return $('.modal.login-modal').modal('hide');
  };

  Home.closeRegisterModal = function() {
    return $('.modal.register-modal').modal('hide');
  };
  ...
  Home.authenticateUser = function() {
    ...
    $.ajax({
      ...
      data: params,
      error: function(data) {
        return Home.handleLoginError(data);
      },
  ...
  Home.registerUser = function() {
    ...
    $.ajax({
      ...
      data: params,
      error: function(data) {
        return Home.handleRegisterError(data);
      },
      success: function(data) {
        return Home.handleRegisterSuccess(data);
      }
    });
    return false;
  };

  Home.handleRegisterSuccess = function(data) {
    if (data["success"] === 'true') {
      $('.register_result').removeClass('login-failed').addClass('login-success');
      $('.register_result').html('Register Sucess!');
      return Home.closeRegisterModal();
    } else {
      $('.register_result').removeClass('login-success').addClass('login-failed');
      return $('.register_result').html(data["reason"]);
    }
  };

  Home.handleRegisterError = function(data) {
    $('.register_result').removeClass('login-success').addClass('login-failed');
    return $('.register_result').html('An error has occured!');
  };

  Home.handleLoginSuccess = function(data) {
    if (data["authenticated"] === 'true') {
      ...
      return $('.login_result').html('Wrong username or password!');
    }
  };

  Home.handleLoginError = function(data) {
    $('.login_result').removeClass('login-success').addClass('login-failed');
    return $('.login_result').html('An error has occured!');
  };

  Home.setCookie = function(name, value, time_in_sec) {
    var d, data;
    d = new Date();
  ...
  Home.readCookie = function() {
    var current_cookie, data, list;
    data = {};
    ...
    _.each(list, function(element) {
      ...
      pairs = element.split('=');
      if (pairs.length === 2) {
        return data[$.trim(pairs[0].toString())] = $.trim(pairs[1].toString());
      }
    });
    return data;
  };
...
(function() {
  ...
  Dashboard.setUp = function() {
    ...
    return $(document).on('click', '.call-button', Dashboard.call);
  };

  Dashboard.openMobileModal = function(event) {
    return $('.test-phone').modal({
      backdrop: 'static',
      keyboard: true
    });
  };

  Dashboard.initTwilioClient = function(data) {
    var token;
    token = data.token;
    return $('#twiloToken').val(token);
  };
...
(function() {
  ...
  $.noty.defaults.layout = 'centerRight';
...
(function() {
  ...
  TwilioDevice = (function() {
    ...
    TwilioDevice.prototype.initTwilioDeviceBindings = function() {
      var twilio_device, twilio_token, url;
      url = "/dashboard/agent";
      $.getJSON(url, {}, function(data) {
        return $('#twilioToken').val(data.token);
      });
      sleep(2000);
      ...
      twilio_device.error(function(error) {
        ...
        if (error.message['code'] === 31205) {
          console.log("TOKEN EXPIRED");
          url = '/dashboard/agent';
          return oo = $.getJSON(url, {});
        }
      });
